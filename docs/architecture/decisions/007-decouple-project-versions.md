...
project versions. More generally, information that is ingested from BOMs stays at the project version level.
...
* Projects without version will no longer be allowed. A `project_version` record without version doesn't make sense.
