...
public class NotificationRuleRepository implements PanacheRepository<NotificationRule> {
    ...
    public List<NotificationRule> findEnabledByScopeAndForLevel(
            final NotificationScope scope,
            final NotificationLevel level) {
        return find(
                "enabled and scope = :scope and notificationLevel <= :level",
                Parameters.with("scope", scope).and("level", level))
