...
class ManagedUserRepositoryTest {
    ...
    void testFindEmailsByTeam() {
        ...
        final var userIds = (List<BigInteger>) entityManager.createNativeQuery("""
                ...
                RETURNING "ID";
                """).getResultList();
        final BigInteger userFooId = userIds.get(0);
        final BigInteger userBarId = userIds.get(1);
        final BigInteger userQuxId = userIds.get(3);
        ...
        final List<String> users = repository.findEmailsByTeam(teamFooId.longValue());
